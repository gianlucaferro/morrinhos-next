...
-- 2. Tabela de registro de jobs (caso ainda não exista vinda de migration anterior).
...
DROP POLICY IF EXISTS "Service role full access" ON public.sync_job_registry;
CREATE POLICY "Service role full access" ON public.sync_job_registry FOR ALL USING (true);

-- 3. Limpar cron jobs anteriores (idempotente — re-aplicar a migration recria tudo).
DO $$
DECLARE
  job_record RECORD;
BEGIN
  FOR job_record IN
    ...
    WHERE jobname LIKE 'sync-%' OR jobname LIKE 'fetch-%' OR jobname LIKE 'send-%'
  LOOP
...
-- =============================================================================
-- 4. AGENDAR CRON JOBS
--    Horário em UTC (Morrinhos = UTC-3).
-- =============================================================================
...
-- Contratos: 3x/semana (segunda, quarta, sexta)
...
SELECT cron.schedule('sync-beneficios-sociais-bw',     '0  3 5,20 * *', $$SELECT public.invoke_edge_function('sync-beneficios-sociais');$$);
...
-- ===================== WEEKLY (terça — dengue, indicadores semanais) =====================
...
SELECT cron.schedule('notify-expiring-ads-daily','0  12 * * *', $$SELECT public.invoke_edge_function('notify-expiring-ads');$$);

-- =============================================================================
-- 5. POPULAR sync_job_registry (idempotente via UPSERT)
-- =============================================================================
...
('sync-contratos-aditivos',    'sync-contratos-aditivos-mon',     '50 4 * * 1', 'weekly',     'Prefeitura + Câmara Centi','Contratos e aditivos (segunda)',                 192, NULL),
('sync-prefeitura-diaria',     'sync-prefeitura-diaria-mon',      '0  5 * * 1', 'weekly',     'Prefeitura Centi',        'Contratos, licitações, servidores (segunda)',     192, NULL),
...
('sync-beneficios-sociais',    'sync-beneficios-sociais-bw',      '0  3 5,20 * *',  'biweekly', 'Portal Transparência',   'Bolsa Família e benefícios',                     384, NULL),
-- Monthly
...
-- Daily
('sync-health-check',          'sync-health-check-daily',         '0  10 * * *', 'daily',       'Health monitoring',     'Health check de todas as functions',             24, NULL)
